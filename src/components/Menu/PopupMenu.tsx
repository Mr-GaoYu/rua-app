...
import {some} from 'src/utilities/ReactChildren';
...
> = React.forwardRef((props: PopupMenuProps, ref: React.Ref<HTMLLIElement>) => {
    const { component: Component, children, onOpenChange, activeKey } = props;

    const isActive = React.useCallback((props) => {
        if (typeof activeKey !== 'undefined' && equals(props.eventKey, activeKey)) {
            return true
        }

        if(some(props.children, child => isActive(child.props))){
            return true;
        }
    ...
    let level = 0;
    const items = React.Children.map(children, (child: any,index:number) => {
        if (!child) {
            return null
        }
